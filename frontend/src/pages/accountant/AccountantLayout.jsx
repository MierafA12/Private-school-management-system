...
// Custom active-check so /accountant/invoices doesn't match /invoices/generate
const isActive = (to, pathname) => {
  if (to === '/accountant/invoices') {
    return pathname === '/accountant/invoices' ||
      (pathname.startsWith('/accountant/invoices/') &&
       !pathname.startsWith('/accountant/invoices/generate'));
  }
  if (to === '/accountant/dashboard') {
    return pathname === '/accountant/dashboard' || pathname === '/accountant';
  }
...
export default function AccountantLayout() {
  ...
  const page   = NAV.find(n => isActive(n.to, location.pathname))?.label
    ...
    <div className="sl-root">
      ...
      <aside className={`sl-sidebar ${open ? 'sl-sidebar--open' : ''}`}>
        <div className="sl-logo">
          ...
          <div><div className="sl-logo-name">EduFlow</div><div className="sl-logo-sub">Finance Portal</div></div>
        ...
        <nav className="sl-nav">
          {NAV.map(({ to, icon: Icon, label }) => (
            <NavLink
              key={to}
              to={to}
              className={() => `sl-nav-item${isActive(to, location.pathname) ? ' sl-nav-item--active' : ''}`}
