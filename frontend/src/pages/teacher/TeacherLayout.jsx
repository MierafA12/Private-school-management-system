...
import {
  ...
  FileText, ClipboardList, User, LogOut, Menu, X, GraduationCap,
} from 'lucide-react';
    ...
    <div className="sl-root">
      ...
      <aside className={`sl-sidebar ${open ? 'sl-sidebar--open' : ''}`}>
        ...
        <div className="sl-sidebar-footer">
          <NavLink to="/teacher/profile" className="sl-user-row" onClick={() => setOpen(false)} style={{ textDecoration: 'none', cursor: 'pointer' }}>
            <div className="sl-avatar">{avatar}</div>
            <div className="sl-user-details"><span className="sl-user-name">{name}</span><span className="sl-user-role">{user?.role || 'Teacher'}</span></div>
      ...
      <div className="sl-main">
        <header className="sl-topbar">
          ...
          <div className="sl-topbar-right">
            ...
            <button className="sl-topbar-avatar-btn" onClick={() => navigate('/teacher/profile')} title="My Profile">{avatar}</button>
          </div>
        </header>
